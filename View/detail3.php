...
<div class="layui-fluid gq-height-100" style="width: 100%;display: flex;flex-direction: column;padding: 0">
    ...
    <div class="layui-container gq-height-80 layui-col-md12 layui-col-xs12 layui-col-sm12 layui-col-lg12" style="width: 100%">

        <!--聊天框-->
        <div class="layui-row gq-row-center gq-height-100">
            <div class="layui-col-md7  layui-col-xs12 layui-col-sm12" id="msg-container" style="background-color: #ededed;padding: 10px;overflow: auto">
            ...
            <div class="layui-form layui-col-md7 layui-col-xs12 layui-col-sm12">
                <div class="layui-row layui-col-md12 layui-col-xs12 layui-col-sm12">
                    <div class="layui-input-group layui-col-md12 layui-col-xs12 layui-col-sm12">
                        <div id="leave-message-textarea" contenteditable="true" data-text="请输入消息内容" style="font-size: 16px"></div>
                        <div class="layui-input-suffix" style="width: 0;height: 34px;line-height: 34px;padding: 0px 0px 0px 10px;">
                            <button id="questionCommit" class="layui-btn layui-btn-primary gq-height-100" style="border-radius: 5px;font-size: 16px" onclick="commit()">发送</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function listLatelyQuestions(){
        ...
        let url = makeRequestUrl('Question','listSameTimeQuestions');
        ...
        $.ajax({
            url:url,
            type:'get',
            data:{num:2,question_id},
            success:function (res) {
                successAfter(res)
                let list = res.obj.questions;
                let datetime = res.obj.datetime;

                if (!isPc()){
                    appendDatetimeToContainer('手机端暂时不支持markdown格式');
                }
                appendDatetimeToContainer(datetime);
    ...
    function getLeftMsgHtml(content){
        ...
        let reply = '';
        if (isPc()){
            reply = marked.parse(content)
        }else{
            reply = content
        }
        // let markHtml = marked.parse(content)
        // let markHtml = content
        ...
        str += '<div class="gq-msg-text">'+reply+'</div>';
        str += '</div>'
        return str;
    }
    ...
    function appendDatetimeToContainer(datetime){
        $('#msg-container').append(getTimeHtml(datetime));
    }
    ...
    function commit(){
        ...
        $.ajax({
            ...
            url:makeRequestUrl('Question','commit'),
            type:'get',
