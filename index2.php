...
log_d(__METHOD__, sprintf('controller:%s, action:%s ,接收参数:%s, jsonBody: %s', $c, $a, _j($otherParams), $jsonBody));
...
if (file_exists($controllerFilePath)) {
    ...
    if ($jsonBody){
        $postArrParams = explode('&', $jsonBody);
        $postParams = [];
        foreach ($postArrParams as $postParam) {
            list($key, $val) = explode('=', $postParam);
            $postParams[$key] = $val;
        }
        $otherParams = array_merge($otherParams, $postParams);
    }
